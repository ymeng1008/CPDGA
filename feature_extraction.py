...
# 最近年份样本
WHITE_FILE_2023 = cp.get('files', 'white_file_2023')
...
def dataset_generation():
    ...
    # 这行代码使用之前创建的 StandardScaler 对象 standardScaler 对训练集特征数据进行归一化处理
...
def new_year_dataset_generation():
    """
    数据集划分,
    :return:
    """
    # 读取白名单（wh_df）数据，这些数据包含了域名和相应的标签信息

    #wh_df = pd.read_csv(WHITE_FILE_2023, header=None)
    ...
    print("__________Generating New year test Set__________")
    
    test_feature = feature_extraction(df_test)
    test_feature.to_csv(r"{}\raw_test_features_2023.csv".format(FEATURE_DIR), index=None)
    test_feature = test_feature.set_index(['domain_name', 'label'])

    # 标准化（归一化）特征数据
    standardScaler = StandardScaler()

    # 在这里，你需要在测试集上拟合StandardScaler对象
    ...
    test_feature = test_feature.reset_index()
    # 将已归一化的测试集特征数据保存为CSV文件
    test_feature.to_csv(r"{}\test_features_2023.csv".format(FEATURE_DIR), index=None)
    pickle.dump(standardScaler, open(r"{}\standardscalar.pkl".format(MODEL_DIR), 'wb'))
    return
