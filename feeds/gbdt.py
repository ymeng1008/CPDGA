...
class GBDTClassifier:
    ...
    def __init__(self):
        self.GBDT_clf = GradientBoostingClassifier(n_estimators=100, learning_rate=0.8)
    ...
    def train(self, model_folder, train_feature_add):
        ...
        print("_______GBDT Training_______")
        self.GBDT_clf.fit(x_train, y_train)
        mal_scores = np.array(self.GBDT_clf.predict_proba(x_train))[:, 1]
        mal_scores = sorted(mal_scores)
        np.save(r"{}/GBDT_train_scores.npy".format(model_folder), mal_scores)
        pickle.dump(self.GBDT_clf, open("{}/GBDT_model.pkl".format(model_folder), 'wb'))
    ...
    def load(self, model_folder):
        ...
        :return:
        """
        self.GBDT_clf = pickle.load(open("{}/GBDT_model.pkl".format(model_folder), 'rb'))
        self.standardScaler = pickle.load(open("{}/standardscalar.pkl".format(model_folder), 'rb'))
        self.train_score = np.load(r"{}/GBDT_train_scores.npy".format(model_folder))
        self.isload_ = True
    ...
    def predict(self, model_folder, test_feature_add):
        ...
        print("_______GBDT Predicting_______")
        y_predict = self.GBDT_clf.predict(x_test)
        print("GBDT accuracy: ", accuracy_score(y_test, y_predict))
        print("GBDT precision: ", precision_score(y_test, y_predict))
        print("GBDT recall: ", recall_score(y_test, y_predict))
        print("GBDT F1: ", f1_score(y_test, y_predict))
    ...
    def predict_single_dname(self, model_folder, dname):
        ...
        if dname == "":
            ...
            label = self.GBDT_clf.predict(feature)
            prob = self.GBDT_clf.predict_proba(feature)
