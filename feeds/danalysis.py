...
class LDAClassifier:
    ...
    def train(self, model_folder, train_feature_add):
        ...
        x_train = x_train.astype(float).astype(int)
...
if __name__ == "__main__":
    """csv_file_path = './Year_Data/2019.csv'
    data_tsne = traindataset_tSNE(csv_file_path)
    data_tsne.to_csv("output_file.csv", index=False)
    """
    clf = LDAClassifier()
    
    clf.train(r"./data/model", r"./feeds/train.csv")
    #clf.train(r"./data/model", r"./1220new/consistent_points_1000.csv")
    #clf.train(r"./data/model", r"./1220new/19bad_good_train.csv")
    
    clf.load(r"./data/model")
    clf.predict(r"./data/model", r"./feeds/test.csv")
